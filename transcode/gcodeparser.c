	...
	if ('G' == *line) {
		  ...
		  case 1:	//move
			  {
				  if (fb)
					  gcp_F = fv;
				  if (eb)
					  gcp_E = (gcp_use_absoulte || gcp_use_extruder_absoulte) ? ev : gcp_E + ev;
				  if (xb)
					  gcp_X = (gcp_use_absoulte) ? xv : gcp_X + xv;
				  if (yb)
					  gcp_Y = (gcp_use_absoulte) ? yv : gcp_Y + yv;
				  if (zb)
					  gcp_Z = (gcp_use_absoulte) ? zv : gcp_Z + zv;
				  if (zb) {
		  ...
		  case 28:	//home
			  {
				  if (!xp && !yp && !zp) {
					  ...
					  yp = true;
					  zp = true;
				  }	//if no parameter given home all axis
				  if (xp) {
					  gcp_X = 0;
				  }
				  if (yp) {
		...
		switch ((int)code) {
		  ...
		  case 104:	//set extruder target temp
			  if (sb)
				  umcwriter_set_extruder_temp(sv, false);
			  break;
		  ...
		  case 109:	//set extrduder target temp and wait
			  if (sb)
				  umcwriter_set_extruder_temp(sv, true);
			  if (rb)
				  umcwriter_set_extruder_temp(rv, true);
			  break;

		  case 140:	//set bed target temp
			  if (sb)
				  umcwriter_set_bed_temp(sv, false);
			  break;

		  case 190:	//set bed target temp and wait
			  if (sb)
				  umcwriter_set_bed_temp(sv, true);
			  if (rb)
				  umcwriter_set_bed_temp(rv, true);
			  break;
