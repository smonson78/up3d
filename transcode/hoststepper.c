...
void _st_create_up3d_seg_c(segment_up3d_t * pseg, double v)
{
	...
	for (; p1 < 65536; p1++) {
		...
		if (pl_block->steps[0] < 0)
			p3 = 0;	//take care not to insert REVERSE steps
		int64_t p4 = (int64_t) (s_y / p1);
		if (pl_block->steps[1] < 0)
			p4 = 0;	//take care not to insert REVERSE steps
		int64_t p5 = (int64_t) (s_a / p1);
		if (pl_block->steps[2] < 0)
			p5 = 0;	//take care not to insert REVERSE steps
...
void st_prep_buffer()
{
	if (segment_buffer_tail != segment_next_head)	// Check if we need to fill the buffer.
	{
					...
					if (prep.decelerate_after < intersect_distance)	// Trapezoid type
					{
						...
						if (pl_block->entry_speed_sqr == pl_block->nominal_speed_sqr) {
							...
							prep.accelerate_until -=
							    inv_2_accel * (pl_block->nominal_speed_sqr - pl_block->entry_speed_sqr);
						}
					} else {
						// Triangle type
		...
		if (pl_block->millimeters - prep.accelerate_until) {
			...
			double time_var =
			    2.0 * (pl_block->millimeters - prep.accelerate_until) / (prep.current_speed + prep.maximum_speed);
