...
char UP3D_STR_MACHINE_STATE[][32] = { "System Error", "Jogging", "Running Program", "Idle", "Unknown Status", "<UNK>" };

char UP3D_STR_PROGRAM_STATE[][32] = { "Program Stop", "Program Running", "Program Pause", "Program Have Errors", "<UNK>" };

char UP3D_STR_SYSTEM_STATE[][32] = { "Unknown", "System Ready", "Running", "Paused", "Nozzle Error", "Motion Error",
...
char UP3D_STR_MOTOR_STATE[][32] = { "STOP", "JOG", "PROG", "ERRORSTOP", "HOMEING", "ERROR", "<UNK>" };

char UP3D_STR_AXIS_NAME[][2] = { "X", "Y", "Z", "A" };
char UP3D_STR_AXIS_STATE[][32] = { "No Error", "+Limit Error", "-Limit Error", "<UNK>" };
...
void UP3D_PROG_BLK_MoveF(UP3D_BLK pupblks[2], float speedX, float posX, float speedY, float posY, float speedZ, float posZ,
			 float speedA, float posA)
{
	memset(pupblks, 0, sizeof(UP3D_BLK) * 2);
	pupblks[0].pcmd = UP3DPCMD_MoveF;
	pupblks[0].pdat1.f = speedX;
	pupblks[0].pdat2.f = posX;
	pupblks[0].pdat3.f = speedY;
	pupblks[0].pdat4.f = posY;
	pupblks[1].pcmd = UP3DPCMD_MoveF;
	pupblks[1].pdat1.f = speedZ;
	pupblks[1].pdat2.f = posZ;
	pupblks[1].pdat3.f = speedA;
	pupblks[1].pdat4.f = posA;
}

void UP3D_PROG_BLK_MoveL(UP3D_BLK * pupblk, uint16_t p1, uint16_t p2, int16_t p3, int16_t p4, int16_t p5, int16_t p6, int16_t p7,
			 int16_t p8)
{
	memset(pupblk, 0, sizeof(UP3D_BLK));
