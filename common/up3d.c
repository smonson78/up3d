...
bool UP3D_IsPrinterResponsive()
{
	...
	if ((UP3DCOMM_Write(UP3D_CMD_1, sizeof(UP3D_CMD_1)) != sizeof(UP3D_CMD_1)) ||
	    (5 != UP3DCOMM_Read(resp, sizeof(resp))) || (6 != resp[4]))
		return false;
...
bool UP3D_InsertRomProgram(uint8_t prog)
{
	...
	if ((UP3DCOMM_Write(UP3D_CMD_4C_X, sizeof(UP3D_CMD_4C_X)) != sizeof(UP3D_CMD_4C_X)) ||
	    (5 != UP3DCOMM_Read(resp, sizeof(resp))) || (6 != resp[4]) || (1 != resp[0])
	    )
		return false;
...
bool UP3D_WriteBlocks(const UP3D_BLK * data, uint8_t blocks)
{
	...
	for (; blocks > 0;) {
		...
		if (_up3d_connected_fw_version < 330)
			if (b > 3)
				b = 3;	//older firmware versions support max 64 bytes (3 blocks) per write only

		if (b > 72)
			b = 72;	//limit to max. 2048 bytes per logical packet
...
bool UP3D_StartResumeProgram()
{
	...
	if ((UP3DCOMM_Write(UP3D_CMD_58, sizeof(UP3D_CMD_58)) != sizeof(UP3D_CMD_58)) || (0 == UP3DCOMM_Read(resp, sizeof(resp)))	/*||
					   (6 != resp[0]) */ )
					//returns "Exe Count: 0" 06
		return false;
	return true;
}
...
bool UP3D_GetPrinterInfo(TT_tagPrinterInfoHeader * pihdr, TT_tagPrinterInfoName * piname, TT_tagPrinterInfoData * pidata,
