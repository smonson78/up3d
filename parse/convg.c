...
typedef struct UPBLOCK {
	uint32_t pcmd;
	union UPBLOCKDAT {
		struct UPBLOCKDATFLOATS {
			float f1;
			float f2;
			float f3;
			float f4;
		} floats;
		struct UPBLOCKDATSHORTS {
			int16_t s1;
			int16_t s2;
			int16_t s3;
			int16_t s4;
...
void _parse_dat_block(UPBLOCK * pBlock)
{
	...
	switch (pBlock->pcmd) {
	  ...
	  case PCMD_MoveF:
		  _dat_cmd_MoveF(pBlock->pdat.floats.f1, pBlock->pdat.floats.f2, pBlock->pdat.floats.f3, pBlock->pdat.floats.f4,
				 _cmd3_XY);
		  _cmd3_XY = !_cmd3_XY;
		  break;
	  ...
	  default:
		  printf(";UNKNOWN CMD: %08X : %08X %08X %08X %08X\n",
			 pBlock->pcmd, pBlock->pdat.longs.l1, pBlock->pdat.longs.l2, pBlock->pdat.longs.l3, pBlock->pdat.longs.l4);
